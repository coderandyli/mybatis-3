...
public class PooledDataSource implements DataSource {
  ...
  // OPTIONAL CONFIGURATION FIELDS (可配置字段)
  /**
   * 最大活跃连接数
   */
  ...
  /**
   * 配置 poolPingQuery 多长时间执行一次，单位 毫秒
   */
  ...
  protected void pushConnection(PooledConnection conn) throws SQLException {
    ...
    synchronized (state) {
      // 先从activeConnections删除此connection
  ...
  private PooledConnection popConnection(String username, String password) throws SQLException {
    ...
    while (conn == null) {
      synchronized (state) {
        ...
        } else { // 如果没有空闲connection
          // Pool does not have available connection
          if (state.activeConnections.size() < poolMaximumActiveConnections) { // 活跃connection数没有达到最大connection数
            // Can create new connection
            conn = new PooledConnection(dataSource.getConnection(), this);
            if (log.isDebugEnabled()) {
              log.debug("Created connection " + conn.getRealHashCode() + ".");
            }
          } else { // 活跃connection数已经达到最大，不能再new了
